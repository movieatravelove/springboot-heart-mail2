var validator;
var $userAddForm = $("#WXUser-add-form");
...
$(function () {
    ...
    $("#WXUser-add .btn-save").click(function () {
        ...
        if (flag) {
            if (name === "save") {
                $.post(ctx + "WXUser/add", $userAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
            ...
            if (name === "update") {
                $.post(ctx + "WXUser/update", $userAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
                        $MB.refreshTable("userTable");
...
function closeModal() {
    ...
    $("#WXUser-add-modal-title").html('新增用户');
    $("#status").html('可用');
