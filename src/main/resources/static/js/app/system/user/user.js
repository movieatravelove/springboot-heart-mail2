$(function () {
    var $userTableForm = $(".WXUser-table-form");
    var settings = {
        url: ctx + "WXUser/list",
        pageSize: 10,
...
function exportUserExcel() {
    $.post(ctx + "WXUser/excel", $(".WXUser-table-form").serialize(), function (r) {
        if (r.code === 0) {
...
function exportUserCsv() {
    $.post(ctx + "WXUser/csv", $(".WXUser-table-form").serialize(), function (r) {
        if (r.code === 0) {
            window.location.href = "common/download?fileName=" + r.msg + "&delete=" + true;
        } else {
            $MB.n_warning(r.msg);
